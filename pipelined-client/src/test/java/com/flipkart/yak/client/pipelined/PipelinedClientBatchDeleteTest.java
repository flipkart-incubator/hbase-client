...
public class PipelinedClientBatchDeleteTest extends PipelinedClientBaseTest {

  private List<DeleteData> dataList;
  ...
  public PipelinedClientBatchDeleteTest(boolean runWithHystrix, boolean runWithIntent) {
  ...
  @Test public void testBatchDeleteToRegion1SiteA() throws ExecutionException, InterruptedException {
    ...
    verify(region1SiteAClient, times(1)).delete(dataList);
    verify(region2SiteAClient, times((runWithIntent) ? 1 : 0)).put(intentStoreData);
  }

  @Test public void testBatchDeleteToRegion1SiteASync()
  ...
  @Test public void testBatchDeleteToRegion1SiteAWithNoIntent() throws ExecutionException, InterruptedException {
    CompletableFuture<PipelinedResponse<List<StoreOperationResponse<Void>>>> future = new CompletableFuture<>();
    CompletableFuture<Void> respFuture = CompletableFuture.completedFuture(null);
    when(region1SiteAClient.delete(eq(dataList))).thenReturn(Arrays.asList(respFuture, respFuture));
  ...
  @Test public void testBatchDeleteToRegion1SiteAWithIntentFailure() throws InterruptedException {
    ...
    try {
      ...
      verify(region1SiteAClient, times(0)).delete(dataList);
    }
    verify(region2SiteAClient, times((runWithIntent) ? 1 : 0)).put(intentStoreData);
  }

  @Test public void testBatchDeleteToRegion2SiteA() throws ExecutionException, InterruptedException {
    CompletableFuture<PipelinedResponse<List<StoreOperationResponse<Void>>>> future = new CompletableFuture<>();
    CompletableFuture<Void> respFuture = CompletableFuture.completedFuture(null);
    ...
    verify(region1SiteAClient, times(0)).delete(dataList);
    verify(region2SiteAClient, times(1)).delete(dataList);
    verify(region2SiteAClient, times((runWithIntent) ? 1 : 0)).put(intentStoreData);
  }

  @Test public void testBatchDeleteToRegion1SiteAWithException() throws ExecutionException, InterruptedException {
    String errorMessage = "Failed";
    CompletableFuture<PipelinedResponse<List<StoreOperationResponse<Void>>>> future = new CompletableFuture<>();
    CompletableFuture respFailureFuture = new CompletableFuture();
    respFailureFuture.completeExceptionally(new PipelinedStoreException(errorMessage));
  ...
  @Test public void testBatchDeleteToRegion1SiteAWithExceptionSync()
          throws ExecutionException, InterruptedException, TimeoutException {
    String errorMessage = "Failed";
    CompletableFuture respFailureFuture = new CompletableFuture();
    respFailureFuture.completeExceptionally(new PipelinedStoreException(errorMessage));
  ...
  @Test public void testBatchDeleteToRegion1SiteAWithNoSitesException() throws InterruptedException {
    CompletableFuture<PipelinedResponse<List<StoreOperationResponse<Void>>>> future = new CompletableFuture<>();
    CompletableFuture<Void> respFuture = CompletableFuture.completedFuture(null);
    when(region1SiteAClient.delete(eq(dataList))).thenReturn(Arrays.asList(respFuture, respFuture));
    store.delete(dataList, Optional.of("Dummy"), intentData, hystrixSettings, responsesHandler(future));
  ...
  @Test public void testBatchDeleteToRegion1SiteAWithSomeSitesInitializeFail() throws Exception {
    ...
    verify(region1SiteAClient, times(0)).delete(dataList);
    verify(region1SiteBClient, times(1)).delete(dataList);
    verify(region2SiteAClient, times((runWithIntent) ? 1 : 0)).put(intentStoreData);
  }

  @Test public void testBatchDeleteToRegion1SiteBAsFallback() throws Exception {
    storeRoute =
        new StoreRoute(Region.REGION_1, ReadConsistency.PRIMARY_MANDATORY, WriteConsistency.PRIMARY_PREFERRED,
            router);
    store = generatePipelinedStore(storeConfig, storeRoute, registry, intentStore);
  ...
  @Test public void testBatchDeleteToRegion1SiteBAsDoubleFallback() throws Exception {
    storeRoute =
        new StoreRoute(Region.REGION_1, ReadConsistency.PRIMARY_MANDATORY, WriteConsistency.PRIMARY_PREFERRED,
            router);
    store = generatePipelinedStore(storeConfig, storeRoute, registry, intentStore);
